...
output_csv_file = '/home/clara/Thesis/strawberry-pp-w-r-dataset-master/compa_paths_a_ga/normalized_error_results.csv'
base_output_plots_directory = '/home/clara/Thesis/strawberry-pp-w-r-dataset-master/compa_paths_a_ga'
    ...
    try:
        ...
        for filename, (ground_truth_file, predicted_file) in associations.items():
            try:
                ...
                scheduling_order_errors = [abs(gt - pred) / pred for gt, pred in zip(ground_truth_scheduling_order, predicted_scheduling_order)]
                ...
                combined_ground_truth_scheduling_orders.extend(ground_truth_scheduling_order)
                combined_predicted_scheduling_orders.extend(predicted_scheduling_order)
                ...
                plt.plot(indices, predicted_total_length, label='GA', color='orange', marker='x')
                plt.xlabel('Index')
                plt.ylabel('Total Length')
                plt.title(f'Total Lengths: A star vs GA  for {filename}', fontsize=16)
                plt.legend()
                plt.grid(True)

                # Line plot for ground truth and predicted scheduling orders
                plt.subplot(2, 1, 2)
                plt.plot(indices, ground_truth_scheduling_order, label='A Star', color='blue', marker='o')
                plt.plot(indices, predicted_scheduling_order, label='GA', color='orange', marker='x')
                plt.xlabel('Index')
                plt.ylabel('Scheduling Order')
                plt.title(f'Scheduling Orders: A Star vs GA for {filename}', fontsize=16)
                plt.legend()
                plt.grid(True)
...
plt.xlabel('Index')
plt.ylabel('Total Length')
plt.title('Total Lengths: A star VS GA ', fontsize=18)
plt.legend()
plt.grid(True)
...
plt.subplot(2, 1, 2)
plt.scatter(combined_indices, combined_ground_truth_scheduling_orders, label='A star', color='blue', marker='o', alpha=0.7)
plt.scatter(combined_indices, combined_predicted_scheduling_orders, label='GA', color='orange', marker='x', alpha=0.7)
plt.axhline(y=average_ground_truth_scheduling_order, color='blue', linestyle='--', linewidth=5, label='Avg A Star Total Length')
plt.axhline(y=average_predicted_scheduling_order, color='orange', linestyle='--', linewidth=5, label='Avg VS GA Total Length')
plt.xlabel('Index')
plt.ylabel('Scheduling Order')
plt.title('Scheduling Orders: A star vs GA', fontsize=18)
plt.legend()
plt.grid(True)

plt.tight_layout()
combined_plot_filename = os.path.join(base_output_plots_directory, 'combined_plot_a_ga.png')
