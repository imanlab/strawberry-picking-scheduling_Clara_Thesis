...
def compute_optimal_path(file_path):
    ...
    # Return the optimal path (the first path in the sorted top ten elements)
    return top_ten_tuples[0][0], G

def process_single_csv(file_path, image_path, output_path):
    # Compute the optimal path
    optimal_path, G = compute_optimal_path(file_path)
    ...
    # Calculate the mid points for the partition
    mid_x = width // 2
    mid_y = height // 2

    # Draw vertical and horizontal lines to partition the image into four equal parts
    image = cv2.line(image, (mid_x, 0), (mid_x, height), (255, 0, 0), 2)
    image = cv2.line(image, (0, mid_y), (width, mid_y), (255, 0, 0), 2)
