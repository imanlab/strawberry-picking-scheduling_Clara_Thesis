...
all_coordinates = get_coordinates('/home/clara/Thesis/strawberry-pp-w-r-dataset-master/scheduling_riseholme/train/coordinates_for_images/113.csv')
...
def visualize_shortest_paths(image, shortest_paths, output_path):
    ...
    # Define the color for the paths (e.g., dark yellow)
    color = (0, 255, 255)  # BGR format for dark yellow
    
    # Define the color for the node numbers (e.g., white)
    number_color = (255, 255, 255)  # BGR format
    ...
    for i in range(len(path_coordinates) - 1):
        ...
        cv2.line(image_copy, start_int, end_int, color, thickness=2)
        
        # Draw the node index as text at the start of each line segment
        cv2.putText(image_copy, str(i), start_int, font, 1, number_color, 2, cv2.LINE_AA)

    # Save the image with the drawn paths
    cv2.imwrite(output_path, image_copy)
    
    # Optionally show the image
    cv2.imshow('Shortest Paths', image_copy)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

# Load your image here
image = cv2.imread('/home/clara/Thesis/strawberry-pp-w-r-dataset-master/scheduling_riseholme/train/strawberry_riseholme_lincoln_tbd_113_rgb.png')

# Path to save the output image
output_image_path = 'shortest_path_image.png'

# Assuming `image` is your OpenCV image and `shortest_paths` is your list of paths
visualize_shortest_paths(image, shortest_paths, output_image_path)
