...
file_path = '/home/clara/Thesis/strawberry-pp-w-r-dataset-master/scheduling_riseholme/train/coordinates_for_images/25.csv'
...
shortest_paths = top_ten_elements
...
subgraphs = [G1, G2, G3, G4]

def visualize_subgraphs_and_centroids_with_shortest_path(image, subgraphs, centroids, shortest_path):
    # Assuming 'image' is a numpy array representing your canvas
    # 'subgraphs' is a list of networkx subgraphs
    # 'centroids' is a list of centroid coordinates
    # 'shortest_path' is a list of node indices representing the shortest path

    # Create a copy of the image to draw on
    image_copy = image.copy()
    shortest_path_data = min(shortest_paths, key=lambda x: x[0][1])
    print(f"shortest_path_data: {shortest_path_data}")
    shortest_path = shortest_path_data[0][0]

    # Define colors
    node_color = (0,  255,  0)  # Green for nodes
    centroid_color = (0,  0,  255)  # Red for centroids
    edge_color = (255,  255,  255)  # White for edges
    path_color = (0,  255,  255)  # Cyan for the shortest path
    circle_color = (0,  255,  0)    # Green for circles


    # Define the font for the node numbers
    ...
    for subgraph_index, subgraph in enumerate(subgraphs):
        ...
        for node in subgraph.nodes:
            x, y, z = subgraph.nodes[node]['pos']
            cv2.circle(image_copy, (int(x), int(y)),  5, node_color, -1)
            cv2.putText(image_copy, str(node), (int(x), int(y)), font,  1, node_color,  2, cv2.LINE_AA)
            
            
    # Iterate over each subgraph and its centroid
    for subgraph, centroid in zip(subgraphs, centroids):
        # Calculate the farthest node from the centroid
        max_distance =  0
        for node in subgraph.nodes:
            node_pos = subgraph.nodes[node]['pos']
            distance = np.linalg.norm(np.array(node_pos) - np.array(centroid))
            if distance > max_distance:
                max_distance = distance

        # Draw a circle centered at the centroid with the radius as the farthest node
        #cv2.circle(image_copy, tuple(np.array(centroid, dtype=int)), int(max_distance), circle_color,  1)
        # Optionally, draw the centroid as a point
        #cv2.circle(image_copy, tuple(np.array(centroid, dtype=int)),  5, centroid_color, -1)
        ...
        for edge in subgraph.edges:
            start_node = edge[0]
            end_node = edge[1]
            start_pos = subgraph.nodes[start_node]['pos']
            end_pos = subgraph.nodes[end_node]['pos']
            cv2.line(image_copy, (int(start_pos[0]), int(start_pos[1])), (int(end_pos[0]), int(end_pos[1])), edge_color,  2)
        
        # Draw centroids
        for centroid_index, centroid in enumerate(centroids):
            x, y, z = centroid  # Assuming 'centroid' is now a  2D point
            cv2.circle(image_copy, (int(x), int(y)),   10, centroid_color, -1)
            # Draw a circle centered at the centroid with the radius as the farthest node
            print(max_distance)
            #cv2.circle(image_copy, (int(x), int(y)), int(distance), circle_color,  1)
            # Optionally, draw the centroid as a point
            #cv2.circle(image_copy, tuple(np.array(centroid, dtype=int)),  5, centroid_color, -1)
            cv2.putText(image_copy, f"C{centroid_index}", (int(x), int(y)), font,   1, centroid_color,   2, cv2.LINE_AA)

        path_coordinates = [(all_coordinates[i][1][0], all_coordinates[i][1][1]) for i in shortest_path]
        ...
        for i in range(len(path_coordinates) -   1):
         ...
         # Draw a line segment for each pair of consecutive points in the path
         cv2.line(image_copy, start_int, end_int, path_color, thickness=2)
       

    # Show the image with the drawn subgraphs, centroids, and shortest path
    cv2.imshow('Subgraphs, Centroids, and Shortest Path', image_copy)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    
# Load your image here
image = cv2.imread('/home/clara/Thesis/strawberry-pp-w-r-dataset-master/scheduling_riseholme/train/strawberry_riseholme_lincoln_tbd_25_rgb.png')
#Assuming `image` is your OpenCV image and `shortest_paths` is your list of paths
visualize_subgraphs_and_centroids_with_shortest_path(image, subgraphs, centroids, shortest_paths)
